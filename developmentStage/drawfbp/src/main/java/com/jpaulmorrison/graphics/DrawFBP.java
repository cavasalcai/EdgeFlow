...
public class DrawFBP extends JFrame
	...
	HashMap<String, String> flows_constraints = new HashMap<>(); // a dictionary to save the flow constraint added by the developer
	...
	FoundPointB fpArrowRoot = null; // this is used to draw blue circle where
									// arrows can start
	FoundPointB fpArrowEndB = null; // this is used to draw black square where
								// arrows can end
	...
	public JMenuBar createMenuBar() {
		...
		menuItem = new JMenuItem("Set flow constraints");
		fileMenu.add(menuItem);
		menuItem.addActionListener(this);
		menuItem = new JMenuItem("Delete flow constraints");
		fileMenu.add(menuItem);
		menuItem.addActionListener(this);
		menuItem = new JMenuItem("Display flows constraints");
		fileMenu.add(menuItem);
		menuItem.addActionListener(this);
		fileMenu.addSeparator();

		menuItem = new JMenuItem("Application model: generate JSON file");
		fileMenu.add(menuItem);
		menuItem.addActionListener(this);
		menuItem = new JMenuItem("Flow Constraints: generate JSON file");
		fileMenu.add(menuItem);
		menuItem.addActionListener(this);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (s.equals("Set flow constraints")){
			String ans_flow = (String) MyOptionPane.showInputDialog(driver,
					"Choose flow and set e2e delay contraint", "Set flow constraint");
			if ((ans_flow != null) && (ans_flow.length() > 0)) {
				String[] input_text = ans_flow.split(":");
				this.flows_constraints.put(input_text[0], input_text[1]);
			}
			return;
		}

		if (s.equals("Delete flow constraints")){
			String ans_flow = (String) MyOptionPane.showInputDialog(driver,
					"Enter flow id to delete", "Delete flow constraint");
			if ((ans_flow != null) && (ans_flow.length() > 0)) {
				this.flows_constraints.remove(ans_flow);
			}
			return;
		}

		if (s.equals("Display flows constraints")){
//			if (!this.flows_constraints.isEmpty()) {
//				MyOptionPane.showMessageDialog(driver,
//						"No flow constraint is set", MyOptionPane.ERROR_MESSAGE);
//				return;
//			}
			displayFlowsConstraints();
			return;
		}


		if (s.equals("Flow Constraints: generate JSON file")){

			Gson flowConstraints = new GsonBuilder().setPrettyPrinting().create();

			FlowConstraintsJSONfile flows = createFlowsObject();

			File newDirectory = new File("results");

			newDirectory.mkdir();

			try (FileWriter writer = new FileWriter("results/flowConstraints.json")){
				flowConstraints.toJson(flows, writer);
			} catch (IOException exp){
				exp.printStackTrace();
				MyOptionPane.showMessageDialog(this, "The file path cannot be found",
						MyOptionPane.ERROR_MESSAGE);
				return;
			}

			MyOptionPane.showMessageDialog(this, "The file flowConstraints.json was generated successfully." +
							" It can be found at: ./results/",
		...
		if(s.equals("Application model: generate JSON file")){

			Gson flowConstraints = new GsonBuilder().setPrettyPrinting().create();

			ApplicationDescriptionJSONfile application = createApplicationObject();

			File newDirectory = new File("results");

			newDirectory.mkdir();

			try (FileWriter writer = new FileWriter("results/applicationModel.json")){
				flowConstraints.toJson(application, writer);
			} catch (Exception exp){
				exp.printStackTrace();
				MyOptionPane.showMessageDialog(this, "The file path cannot be found",
						MyOptionPane.ERROR_MESSAGE);
			...
			MyOptionPane.showMessageDialog(this, "The file applicationModel.json was generated successfully." +
							" It can be found at: ./results/",
					MyOptionPane.INFORMATION_MESSAGE);
	 ...
	 FlowConstraintsJSONfile createFlowsObject(){

		FlowConstraintsJSONfile file = new FlowConstraintsJSONfile();
		List<FlowConstraintsInputFile> flows = new ArrayList<>();
		Map<String, List<FlowConstraintsInputFile>> flow_constr = new HashMap<>();

		for(String key : this.flows_constraints.keySet()){
			FlowConstraintsInputFile flow = new FlowConstraintsInputFile();
			flow.setId(key);
			String[] value = this.flows_constraints.get(key).split("<=");
			flow.setE2e(value[1]);

			String[] pth = value[0].split("->");

			int path_length = pth.length;
			List<FlowConstraintPathComponent> flow_path = new ArrayList<>();

			for(int i = 0; i < path_length; i++){
				FlowConstraintPathComponent path = new FlowConstraintPathComponent();
				List<Map> ports = new ArrayList<>();
				Map<String, String> inputPort = new HashMap<>();
				Map<String, String> outputPort = new HashMap<>();

				String[] portsAndComp = pth[i].split(" ");
				if(portsAndComp.length == 1) {
//					path.setSrc(portsAndComp[0]);
//					inputPort.put("input", "");
//					outputPort.put("output", "");
					continue;
				}
				else{
					path.setSrc(portsAndComp[1]);
				}

				if(portsAndComp.length == 2){
					path.setDest("");
					inputPort.put("input", portsAndComp[0]);
					outputPort.put("output", "");
				}
				else {
					int next_compt = i + 1;
					String[] dest_portsAndComp = pth[next_compt].split(" ");
					path.setDest(dest_portsAndComp[1]);
					inputPort.put("input", portsAndComp[0]);
					outputPort.put("output", portsAndComp[2]);
			...
			flow.setPath(flow_path);
			flows.add(flow);

		}
		flow_constr.put("flows", flows);
		file.setFlows(flow_constr);

		return file;

	}

	ApplicationDescriptionJSONfile createApplicationObject(){
		ApplicationDescriptionJSONfile file = new ApplicationDescriptionJSONfile();
		List<ApplicationDescriptionComponent> components = new ArrayList<>();
		Map<String, List<ApplicationDescriptionComponent>> app_descr = new HashMap<>();

		for(Block block : curDiag.blocks.values()){

			if(block.type.equals(Block.Types.IIP_BLOCK)){
				continue;
			}

			ApplicationDescriptionComponent component = new ApplicationDescriptionComponent();
			List<Map<String, String>> wcet = new ArrayList<>();
			Map<String, String> wcet_low = new HashMap<>();
			Map<String, String> wcet_up = new HashMap<>();

			List<Map<String, Map<String, String>>> op_ports = new ArrayList<>();
			Map<String, List<Map<String, Map<String, String>>>> output = new HashMap<>();
			Map<String, List<Map<String, Map<String, String>>>> input = new HashMap<>();
			List<Map<String, List<Map<String, Map<String, String>>>>> ports_list = new ArrayList<>();

//			String id = block.desc.replaceAll("\\s+", "_");
			String id = block.desc.split(":")[0];
			component.setId(id);
			component.setCPU(Integer.toString(block.getCPU()));
			component.setRAM(Integer.toString(block.getRAM()));
			component.setHDD(Integer.toString(block.getHDD()));
			wcet_low.put("l", Integer.toString(block.getWCET_low()));
			wcet_up.put("u", Integer.toString(block.getWCET_up()));
			wcet.add(wcet_low);
			wcet.add(wcet_up);
			component.setWCET(wcet);

			for(AOutPort op : block.outputPortAttrs.values()){ 
				Map<String, String> output_ports_values = new HashMap<>();
				Map<String, Map<String, String>> output_ports = new HashMap<>();
				output_ports_values.put("period", Integer.toString(block.getPeriod()));
				output_ports_values.put("data_size", Integer.toString(block.getDataSize()));
				output_ports.put(op.value, output_ports_values);
				op_ports.add(output_ports);
			}
			input.put("input", new ArrayList<>());
			output.put("output", op_ports);
			ports_list.add(input);
			ports_list.add(output);
			component.setPorts(ports_list);
			components.add(component);
		}
		app_descr.put("components", components);
		file.setApplication(app_descr);
		return file;
	}
	...
	void displayFlowsConstraints(){
		...
		jf.setTitle("List of flows constraints");
		...
		JTextField[] tft = new JTextField[PROPERTIES];
		tft[0] = new JTextField(" ID ");
		tft[1] = new JTextField(" path ");
		tft[2] = new JTextField(" e2e delay ");

		tft[0].setFont(fontg);
		tft[1].setFont(fontg);
		tft[2].setFont(fontg);

		displayRow(gbc, gbl, tft, panel, Color.BLUE, PROPERTIES, false);

		for(String key : this.flows_constraints.keySet()){
			String e2e = this.flows_constraints.get(key);
			String[] text = e2e.split("<=");
			JTextField[] tfv = new JTextField[PROPERTIES];
			tfv[0] = new JTextField(String.valueOf(key));
			tfv[1] = new JTextField(String.valueOf(text[0]));
			tfv[2] = new JTextField(String.valueOf(text[1]));

			displayRow(gbc, gbl, tfv, panel, Color.BLACK, PROPERTIES, false);
		}
	...
	void displayProperties() {
		...
		displayRow(gbc, gbl, tft, panel, Color.BLUE, 4, false);
		...
		for (String p : propertyDescriptions.keySet()) {
			...
			while (true) {
				...
				displayRow(gbc, gbl, tft, panel, Color.BLACK, 4, false);
	...
	*/
	...
	void displayRow(GridBagConstraints gbc, GridBagLayout gbl, JTextField[] tf,
					JPanel panel, Color col, int row_size, boolean setEdit) {
		gbc.gridx = 0;
		gbc.weightx = 0.5;
	...
	*/
	...
	public class SelectionArea extends JPanel implements MouseInputListener {
		...
		FoundPointB findBlockEdge(int xa, int ya) {
			...
			for (Block block : curDiag.blocks.values()) {
				...
				if (!(between(xa, block.leftEdge - zWS / 2,
						block.rgtEdge + zWS / 2)))
					continue;

				if (!(between(ya, block.topEdge - zWS / 2,
						block.botEdge + zWS / 2)))
					continue;
