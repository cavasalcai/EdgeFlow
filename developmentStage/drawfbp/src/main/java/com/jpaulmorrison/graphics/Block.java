...
public class Block implements ActionListener {
	...
	int WCET_low = 0; // unit is ms
	int WCET_up = 0;
	int CPU = 0;
	int RAM = 0;
	int HDD = 0;
	int dataSize = 0; // unit is bytes
	int period = 0; // unit is ms
	...
	void displayPortInfo() {
		...
		*/
		...
		displayRow(gbc, gbl, tft, panel, Color.BLUE, ROWSIZE, false);
		...
		for (AInPort ip : inputPortAttrs.values()) {
			...
			gbc.gridx = 0;
			gbc.weightx = 0.5;
			displayRow(gbc, gbl, tfi, panel, Color.BLACK, ROWSIZE, false);
		}
		...
		for (AOutPort op : outputPortAttrs.values()) {
			...
			displayRow(gbc, gbl, tfo, panel, Color.BLACK, ROWSIZE, false);
		}
		...
		for (String ls : lst) {
			...
			displayRow(gbc, gbl, tfu, panel, Color.BLACK, ROWSIZE, false);
		}
...
//		buildMetadata();
		...
		jdialog.setTitle("Component Information");
		jdialog.toFront();
		JPanel panel = new JPanel(new GridBagLayout());
		...
		JTextField tf2 = new JTextField("Component requirements and their values");
		tf2.setEditable(false);
		gbl.setConstraints(tf2, gbc);
		tf2.setBackground(DrawFBP.lg);
		panel.add(tf2);
		...
		tft[0] = new JTextField(" WCET lower bound [ms]");
		tft[1] = new JTextField(" WCET upper bound [ms]");
		tft[2] = new JTextField(" CPU [MIPS]");
		tft[3] = new JTextField(" RAM [GB]");
		tft[4] = new JTextField(" HDD [GB]");
		tft[5] = new JTextField(" Period [ms]");
		tft[6] = new JTextField(" Output Data Size [bytes]");
		displayRow(gbc, gbl, tft, panel, Color.BLACK, PROPERTIES, false);

		JTextField[] tfv = new JTextField[PROPERTIES];
		tfv[0] = new JTextField(String.valueOf(this.getWCET_low()));
		tfv[1] = new JTextField(String.valueOf(this.getWCET_up()));
		tfv[2] = new JTextField(String.valueOf(this.getCPU()));
		tfv[3] = new JTextField(String.valueOf(this.getRAM()));
		tfv[4] = new JTextField(String.valueOf(this.getHDD()));
		tfv[5] = new JTextField(String.valueOf(this.getPeriod()));
		tfv[6] = new JTextField(String.valueOf(this.getDataSize()));
	...
	void setComponentRequirements(){
		...
		int PROPERTIES = 6;
		while(true) {
			String ans_wcet = (String) MyOptionPane.showInputDialog(driver,
					"Set WCET range as follow: lower_upper [ms]", "Set Component Requirements",
					MyOptionPane.PLAIN_MESSAGE, null, null, mpxfactor);
			if ((ans_wcet != null) && (ans_wcet.length() > 0)) {
				String[] wcet = ans_wcet.split("_");
				if (wcet.length == 2 && ans_wcet.contains("_")) {
					this.WCET_low = Integer.parseInt(wcet[0]);
					this.WCET_up = Integer.parseInt(wcet[1]);
					break;
				} else {
					MyOptionPane.showMessageDialog(driver, "WCET range incorrect, please use lower_upper format!!",
		...
		String ans_CPU = (String) MyOptionPane.showInputDialog(driver,
				"Set CPU [MIPS]", "Set Component Requirements",
				MyOptionPane.PLAIN_MESSAGE, null, null, mpxfactor);
		if ((ans_CPU != null) && (ans_CPU.length() > 0)) {
			this.CPU = Integer.parseInt(ans_CPU);
		}

		String ans_RAM = (String) MyOptionPane.showInputDialog(driver,
				"Set RAM [GB]", "Set Component Requirements",
				MyOptionPane.PLAIN_MESSAGE, null, null, mpxfactor);
		if ((ans_RAM != null) && (ans_RAM.length() > 0)) {
			this.RAM = Integer.parseInt(ans_RAM);
		}

		String ans_HDD = (String) MyOptionPane.showInputDialog(driver,
				"Set HDD [GB]", "Set Component Requirements",
				MyOptionPane.PLAIN_MESSAGE, null, null, mpxfactor);
		if ((ans_HDD != null) && (ans_HDD.length() > 0)) {
			this.HDD = Integer.parseInt(ans_HDD);
		}

		String ans_P = (String) MyOptionPane.showInputDialog(driver,
				"Set Period [ms]", "Set Component Requirements",
				MyOptionPane.PLAIN_MESSAGE, null, null, mpxfactor);
		if ((ans_P != null) && (ans_P.length() > 0)) {
			this.period = Integer.parseInt(ans_P);
		}

		String ans_data = (String) MyOptionPane.showInputDialog(driver,
				"Set Output Data Size [bytes]", "Set Component Requirements",
				MyOptionPane.PLAIN_MESSAGE, null, null, mpxfactor);
		if ((ans_data != null) && (ans_data.length() > 0)) {
			this.dataSize = Integer.parseInt(ans_data);
		}
		...
		if (this instanceof Enclosure) {
			...
			if (this instanceof ProcessBlock || this instanceof FileBlock
				...
				if (this instanceof ProcessBlock) {
					...
					diag.jpm.addSeparator();
					menuItem = new JMenuItem("Set component requirements");
					menuItem.addActionListener(this);
					diag.jpm.add(menuItem);
					...
					JMenuItem menuItem4 = new JMenuItem("Display Component Info");
					menuItem4.addActionListener(this);
					diag.jpm.add(menuItem4);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (s.equals("Set component requirements")) {
		...
		if (s.equals("Display Component Info")) {
//			if (javaComp == null) {
//				MyOptionPane.showMessageDialog(driver,
//						"No class information associated with block", MyOptionPane.ERROR_MESSAGE);
//				return;
...
*/
	...
	//create a function similar to this for WCET and add it in the menu here!!!!
